...
Check if docker images are existing in github
https://github.com/meena13oct?tab=packages
curl -v telnet://65.0.81.204:22
ssh -i D:\AWS-CLI\secureclaims-keypair.pem ec2-user@13.206.205.79
aws ec2 create-key-pair --key-name secureclaims-keypair --query "KeyMaterial" --output text --region ap-south-1 | Out-File -Encoding ascii  D:\AWS-CLI\secureclaims-keypair.pem
 curl http://13.206.205.79:8081/actuator/health
 
 First get your VPC ID:
 aws cloudformation describe-stacks --stack-name secureclaims-ai --region ap-south-1 --query "Stacks[0].Outputs[?OutputKey=='VpcId'].OutputValue"  --output text

Check the Network ACLs for that VPC:
aws ec2 describe-network-acls --filters "Name=vpc-id,Values=vpc-0bc5c06837b9d9d80" --region ap-south-1 --query "NetworkAcls[0].[Entries]" --output table

Check security group rules:
aws ec2 describe-security-groups --filters "Name=tag:Name,Values=secureclaims-ec2-sg" --region ap-south-1 --query  "SecurityGroups[0].IpPermissions[].[FromPort,ToPort,IpRanges[0].CidrIp]" --output table

Instance Status
-------------------
aws ec2 describe-instance-status --instance-ids i-0e8c0a5d3faa2cdab --region ap-south-1 --query  "InstanceStatuses[0].[InstanceState.Name,InstanceStatus.Status,SystemStatus.Status]" --output text
aws ec2 describe-instance-status --filters "Name=instance-state-name,Values=running" --region ap-south-1 --query "InstanceStatuses[*].[InstanceId,InstanceState.Name,InstanceStatus.Status,SystemStatus.Status]" --output table
